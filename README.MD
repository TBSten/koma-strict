...
（KMP プロジェクトの場合は [KSP の制限を回避するためのワークアラウンド](https://github.com/TBSten/cream/blob/main/doc/customization/multiplatform.ja.md) が必要です）

また IDE 拡張機能をインストールすることで、`@StoreSpec` を IDE 上で可視化したり、追加のコード生成機能などを利用することができます。[最新リリース](https://github.com/TBSten/koma-strict/releases/latest) から `koma-strict-idea-<version>.zip` をダウンロードして、お使いの IDE の **Settings → Plugins → ⚙ → Install Plugin from Disk…** から選択してインストールしてください（IntelliJ IDEA / Android Studio **2026.1 以降**が必要です）。
...
snapshot は ./koma-strict-ksp/snapshots に格納されており、 特に [koma-strict-ksp/snapshots/StoreSpecUseCasesTest/samples.md のユースケースとオプションの全組み合わせ/option=Default](./koma-strict-ksp/snapshots/StoreSpecUseCasesTest/samples.md%20のユースケースとオプションの全組み合わせ/option=Default) は実際のユースケースを想定した入出力がまとまっているため、これを参照して以下のような具体的なユースケース別の生成例を確認できます。
...
<video src="https://github.com/user-attachments/assets/6f857454-1bd9-4c58-b348-a76b2b97df19" width="80%" controls></video>
