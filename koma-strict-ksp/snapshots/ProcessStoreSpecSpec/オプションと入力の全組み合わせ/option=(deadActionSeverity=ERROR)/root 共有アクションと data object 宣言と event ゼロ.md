 ...
 * `@StoreSpec(initial = ...)` candidate [TabsState.Home] — compile-time enforced.
 * [configuration] appends raw koma DSL after the generated handlers (store-level escape hatch).
 */
public fun createTabsStore(
    initialState: TabsState.Home,
    default: TabsStateDefaultHandlersScope.() -> TabsStateDefaultHandlers,
    search: SearchHandlersScope.() -> SearchHandlers,
 ...
 *
 * Sugar over the canonical koma entry point — `Store<TabsState, TabsAction, Nothing>(initialState) { states(...) }`
 * builds the exact same store. [initialState] accepts any [TabsState] — for
 * restoring a persisted state or starting mid-flow in tests, where [createTabsStore]'s
